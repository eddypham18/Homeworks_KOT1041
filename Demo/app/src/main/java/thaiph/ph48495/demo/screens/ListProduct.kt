...
import thaiph.ph48495.demo.models.foodList
...
fun ListProduct(navController: NavController, viewModel: ViewModelApp = viewModel()) {
    ...
    DialogDelete(
        isShowDialogDelete,
        onConfirm = {
            isSelected?.let { viewModel.deleteFood(it.id) }
            Log.d(TAG, "ListProduct: "+isSelected?.id)
    ...
    isSelected?.let {
        ...
        isShowDialogEdit,
        onConfirm = { newFood ->
            foodList.set(foodList.indexOf(isSelected), newFood)
    ...
    LazyColumn {
        listFood?.let {
            items(it.size) { index ->
                Card(onClick = {
                    navController.currentBackStackEntry?.savedStateHandle?.set("food", foodList[index])
                    ...
                    Row(modifier = Modifier
                        ...
                        Column (modifier = Modifier.padding(start = 10.dp)){
                            Text(text = foodList[index].name)
                            Text(text = foodList[index].price)
                        }
